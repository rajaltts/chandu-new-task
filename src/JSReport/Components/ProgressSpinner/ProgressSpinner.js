...
import { ReportsCircularProgress as CircularProgress, usePercentageStyles } from './ProgressSpinner.styles'
...
const ReportsCircularProgress = ({
    ...
    const [progress, setProgress] = useState(
        status === DOWNLOADING ? 0 : Math.floor((new Date().getTime() - new Date(creationDate).getTime()) / 1000) * 10
    )
        ...
        if (mountRef.current) {
            const timer = setInterval(() => {
                setProgress((prevProgress) =>
                    prevProgress >= 98 ? 99 : prevProgress + percentageIncrement * speedFactor(prevProgress)
                )
            }, 100)
            return () => {
                clearInterval(timer)
    ...
    return (
        <>
            <CircularProgress status={status} value={progress} size={30} variant='determinate' color='secondary' />
            <div className={classes.percentageValue}>{`${Math.round(progress)}%`}</div>
        </>
    )
}
