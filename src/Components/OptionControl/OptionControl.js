...
function OptionControl(props) {
    ...
    const EnabledPropValue =
        props.enableDefaultOptions && EnabledProp && EnabledProp.Value.toUpperCase() === 'FALSE' ? false : true
    ...
    function ValueChanged() {
        if (!Loading && EnabledPropValue) {
            ...
            if ((props.children || InputBlock) && Value === 'FALSE') SetDisplayDetails(true)
            else SetDisplayDetails(false)
            let NewAssignments = []
            if (GetPrice()) {
    ...
    function GetOptionDescription(MainProp) {
        return props.DoNotTranslate ? (
            MainProp.Values[0].Attributes && MainProp.Values[0].Attributes.Description
        ) : (
            <Culture
                id={FormatTransKey(props.PropName)}
                defaultMessage={formatMessage({
                    id: FormatTransKey(
                        props.PropName + 'DESCRIPTION' + '|' + MainProp.Values[0].Attributes.Description
                    ),
    ...
    function GetDetailedDescription(MainProp) {
        return props.DoNotTranslate ? (
            MainProp.Values[0].Attributes && MainProp.Values[0].Attributes.DetailledDescription
        ) : (
            <Culture
                id={FormatTransKey(props.PropName + '.DETAILEDDESCRIPTION')}
                defaultMessage={formatMessage({
                    id: FormatTransKey(
                        props.PropName +
                            'DETAILEDDESCRIPTION' +
                            '|' +
                            MainProp.Values[0].Attributes.DetailledDescription
                    ),
                })}
            />
        )
    ...
    function GetAdvantage(MainProp) {
        return props.DoNotTranslate ? (
            MainProp.Values[0].Attributes && MainProp.Values[0].Attributes.Advantage
        ) : (
            <Culture
                id={FormatTransKey(props.PropName + '.ADVANTAGE')}
                defaultMessage={formatMessage({
                    id: FormatTransKey(props.PropName + 'ADVANTAGE' + '|' + MainProp.Values[0].Attributes.Advantage),
                })}
            />
        )
    }
                    ...
                    <div className='OptionControl-MainContainer'>
                        ...
                        >
                            {Loading ? (
                                ...
                                <Checkbox
                                    color='primary'
                                    className={classNames('OptionControl-Checkbox', {
                                        OptionControlNotEnabled: !EnabledPropValue,
                                    })}
                            ...
                            <div className='OptionControl-LabelsContainer'>
                                ...
                                <span className='OptionControl-OptNumber'>
                                    {MainProp.Values[0].Attributes.OptNumber}
                                </span>
                            </div>
                            {Allowed ? (
                                <div className='OptionControl-CompIcon'>
                                    <Incomp color='#FF9900' width='18px' />
                                </div>
                            ) : null}
                            <Tooltip
                        ...
                        <Tooltip title={<Culture id={'GetMoreInfoOnOption'} />} placement='top'>
                            <div className='OptionControl-InfoIcon' onClick={onInfoIconClick}>
                                <InfoIcon id={props.PropName} color='#2d4181' width='26px' />
                            </div>
                        </Tooltip>
                    </div>
