...
function SimpleCombobox(props) {
    ...
    function UpdateState() {
        ...
        if (VisibleProp === undefined) setVisible(true)
        else setVisible(VisibleProp.Value === 'FALSE' ? false : true)
        let EnabledProp
        ...
        if (EnabledProp === undefined) setEnabled(true)
        else setEnabled(EnabledProp.Value === 'FALSE' ? false : true)
        let ValidProp
        if (props.Valid) ValidProp = GetProperty(props.Valid)
        if (ValidProp === undefined) ValidProp = GetProperty(props.PropName + '.VALID')
        if (ValidProp === undefined) setValid(true)
        else setValid(ValidProp.Value === 'FALSE' ? false : true)
    }
    ...
    if (Visible) {
        return (
            <Fragment>
                {props.isValidationMessage && Valid === false && (
                    <span className='ErrorText'> {props.isValidationMessage} </span>
                )}
                        ...
                        >
                            {search && Open ? (
                                ...
                                <React.Fragment>
                                    <span>
                                        {GetSelectedValue()}
                                        {prop && prop.Value && props.PriceDollar && (
                                            <span>{GetPriceDollar(GetSelectedOption().Attributes.MLP)}</span>
                                        )}
                                    </span>
                                    <div>
                                        {props.Prices && props.Prices.length > 0 && (
                                            <span className='SCB-Price'>{GetPrice(GetSelectedValue())}</span>
                                        )}
                        ...
                        {Open && (
                            <div className={classNames('SCB-SubBtnWrapper', search && 'SCB-input-searchable-list')}>
                                {(sort ? sortValues(prop.Values) : prop.Values).map((value, index) => {
                                    if (
                                        (props.HideNotAllowedValues && value.State === 2) ||
                                        ...
                                        editedValue &&
                                        (filter === 'startsWith'
                                            ? !value.Attributes.Description.toLowerCase().startsWith(
                                                  editedValue.toLowerCase()
                                              )
                                            : !value.Attributes.Description.toLowerCase().includes(
                                                  editedValue.toLowerCase()
                                              ))
                                    )
                                        return null
                                        ...
                                        return (
                                            ...
                                            >
                                                <span>
                                                    ...
                                                    {props.PriceDollar ? (
                                                        <span>{GetPriceDollar(value.Attributes.MLP)}</span>
                                                    ) : null}
                                                </span>
                                                {props.Prices && props.Prices.length > 0 ? (
