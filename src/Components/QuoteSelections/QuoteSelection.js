...
const QuoteSelection = (props) => {
    ...
    const {
        ...
        createNew, opportunityName, selectMenu, selectedMenu
    } = useStyles();
                ...
                <FormControl variant="outlined" size="small" className={formControl}>
                    ...
                    >
                        ...
                        {newQuotation === newLabel &&
                            <MenuItem className={selectedMenu} value={value}>{value}</MenuItem>
                        }
                        {menuList.map(menuItem => {
                            const { name, id } = menuItem
                            return <MenuItem id={id} className={name === value ? selectedMenu : ''} value={name}>{name}</MenuItem>
                        })}
                    </Select>
                </FormControl>
            )
...
export default injectIntl(
    connect(null, { closeQuoteSelection })(
        memo(QuoteSelection)
    )
);