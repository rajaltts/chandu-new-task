...
const ProjectTagSelection = (props) => {
    ...
    useEffect(() => {
        if (projectDataList.length) {
            setDisplayProjectNames(
                projectDataList.filter((project) => project.ProjectName && project.ProjectName.trim() !== '')
            )
        }
    ...
    useEffect(() => {
        ...
        const tagNameStatus = (!isDisabled && !tagNameForCopySelection.trim()) || !!tagError
    ...
    const renderProjectNameOptions = (option, selected) => (
        <React.Fragment>
            <span className={selected ? classes.menuItemSelected : ''}>{option.ProjectName}</span>
        </React.Fragment>
    )
    ...
    const getProjectOptionSelected = (option, value) => option.ProjectName === value.ProjectName

    return (
        <Fragment>
            {!saveSelection ? <TagName tagNameProps={getTagNameProps()} /> : null}
            <div className={classnames(classes.infoProject, classes.helperText)}>
                {injectIntlTranslation(intl, 'LastEdited100Project', 'Last 100 edited projects')}
            </div>
            <Autocomplete
                id='search-project'
                ...
                getOptionSelected={(option, value) => getProjectOptionSelected(option, value)}
                renderOption={(option, { selected }) => renderProjectNameOptions(option, selected)}
                onChange={(event, value, changeType) => onProjectSelectChange(event, value, changeType)}
                ...
                renderInput={(params) => (
                    <TextField
                        {...params}
                        label={
                            <span>{injectIntlTranslation(intl, 'SearchProject', 'Search project here') + ' *'}</span>
                        }
                        size='small'
                        variant='outlined'
            ...
            />
            <span className={classes.errorMsg}>{saveSelection ? projectError : selectProjectError || ''}</span>
        </Fragment>
    )
}
