...
const HorizontalRadioGroupWithImage = (props) => {
    ...
    const handleChange = (value, index) => {
        onValueChanged([{ Name:ValueProp.Name, Value: value.Value === ValueProp.AssignedValue && unCheckRadio ? "NONE" : ValueProp.Values[index].Value }])
    }
                    ...
                    if(value.Attributes.VISIBLE === "TRUE"){
                        return(
                            <div 
                                ...
                                key={index} 
                                className={`IWLCG-tile ${value.State === 1 ? `` : isNotAllowedVisible ?"IWLCG-notAllow":`disabled`}`} 
